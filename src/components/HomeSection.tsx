...
export default function HomeSection({ onNavigate, onOpenConsultation }: HomeSectionProps) {
  ...
  return (
    <div className="relative w-full overflow-hidden bg-black text-white selection:bg-[#22c55e]/30 selection:text-white">
      ...
      {/* ========================================================================= */}
      {/* 8. SECTION: PLANOS & PREÇOS (COMPACT & ALIGNED SIDE-BY-SIDE ON MOBILE) */}
      {/* ========================================================================= */}
      <section id="planos" className="relative mx-auto max-w-6xl px-4 py-16 sm:py-28 border-t border-neutral-900/80">
        ...
        {/* Plans Cards Grid Container - Compact & Full View */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 items-stretch mx-auto bg-gradient-to-b from-neutral-900/60 via-neutral-900/30 to-black/80 sm:p-5 p-3 rounded-2xl sm:rounded-3xl border border-neutral-800/80">
          {content.plans.map((plan, index) => {
            const isPopular = plan.popular;
            const priceData = parsePriceData(billingCycle === 'annual' ? plan.annualPrice : plan.monthlyPrice);

            return (
              <ScrollReveal key={plan.id || index} delay={index * 0.08} duration={0.4} once={true} className="h-full">
                <div 
                  className={cn(
                    "rounded-2xl sm:rounded-3xl p-4.5 sm:p-6 flex flex-col justify-between relative transition-all duration-300 h-full",
                    isPopular
                      ? "scale-100 md:scale-[1.02] z-10 ring-2 ring-[#22c55e] bg-gradient-to-t from-black via-neutral-950 to-[#07170c] text-white shadow-[0_0_40px_rgba(34,197,94,0.22)]"
                ...
                >
                  ...
                  {isAdmin && (
                    <div className={cn(
                      "absolute top-2.5 right-2.5 flex items-center gap-1.5 z-30 transition-opacity",
                  ...
                  <div>
                    ...
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      {(isPopular || plan.badge) && (
                        <span className={cn(
                          "px-2.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider shadow-md shrink-0 whitespace-nowrap",
                          isPopular 
                    ...
                    {/* Description */}
                    <p className={cn(
                      "text-xs mb-4 leading-relaxed min-h-[34px]",
                    ...
                    <div className="flex items-baseline mb-4 pb-4 border-b border-neutral-800/80">
                      {priceData.hasNumber ? (
                        <div className="flex items-baseline">
                          ...
                          />
                          {priceData.suffix && (
                            <span className="text-base sm:text-lg font-bold text-white ml-1">
                              {priceData.suffix}
                            </span>
                          )}
                        </div>
                    ...
                    <div className="space-y-2">
                      <h4 className="font-bold text-[11px] sm:text-xs uppercase tracking-wider text-neutral-200">
                      ...
                      <ul className="space-y-2 pt-0.5">
                        {plan.features.map((feature, fIdx) => (
                          <li key={fIdx} className="flex items-start">
                            <span
                              className={cn(
                                "h-5 w-5 rounded-full grid place-content-center mt-0.5 mr-2.5 shrink-0",
                                isPopular
                                  ? "bg-[#22c55e] text-black shadow-[0_0_10px_rgba(34,197,94,0.35)]"
                  ...
                  {/* CTA Button */}
                  <div className="mt-5 sm:mt-6 pt-3">
                    <button
                      onClick={() => {
                        if (onOpenConsultation) {
                      ...
                      className={cn(
                        "w-full py-3.5 px-4 text-xs sm:text-sm rounded-xl font-black uppercase tracking-wider transition-all duration-200 active:scale-[0.98] cursor-pointer",
                        isPopular
                          ? "bg-[#22c55e] hover:bg-[#16a34a] text-black shadow-[0_0_25px_rgba(34,197,94,0.3)] border border-[#22c55e]"
                          : plan.buttonVariant === 'default'
                            ? "bg-[#22c55e] hover:bg-[#16a34a] text-black font-black border border-[#22c55e]"
